...
re_vals = collect(-.53:0.03:4.73)
im_vals = collect(-.51:0.01:1.61)
...
CSV.write("le_chen.csv", DataFrame(re=real.(es), im=imag.(es), χ=χe))
...
dos = 1/2π * Laplacian(χe, 0.03, 0.01)
idos = sum(dos, dims=2)[:, 1] * 0.03

CSV.write("idos_chen.csv", DataFrame(ime=im_vals[2:end-1], idos=idos))
