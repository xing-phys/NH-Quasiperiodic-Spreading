...
re_vals = collect(.899:0.001:1.101)
im_vals = collect(0.349:0.001:0.551)
...
DataFrame(re=real.(es), im=imag.(es), χ=χe)

CSV.write("le_detailed_0.5.csv", DataFrame(re=real.(es), im=imag.(es), χ=χe))
...
CSV.write("idos_detailed_0.5.csv", DataFrame(ime=im_vals[2:end-1], idos=idos))
